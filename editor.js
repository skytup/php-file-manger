setTimeout(() => {
  SKY.append_line_numbers('input_wrap')
...
const SKY = {
  ...
  update_line_numbers: function(ta, el) {
    ...
    if(difference > 0) {
      ...
      while(difference > 0) {
        // Create a <span>, add SKY class name, append to fragment and
        // update difference
        const line_number = document.createElement("span");
        line_number.className = "sky-line";
  ...
  append_line_numbers: function(id) {
    ...
    if(ta == null) {
      return console.warn("[sky.js] Couldn't find textarea of id '"+id+"'");
    }
    // If <textarea> already has SKY active, warn and leave
    if(ta.className.indexOf("sky-active") != -1) {
      return console.warn("[sky.js] textarea of id '"+id+"' is already numbered");
    }
    // Otherwise, we're safe to add the class name and clear inline styles
    ta.classList.add("sky-active");
    ta.style = {};

    // Create line numbers wrapper, insert it before <textarea>
    const el = document.createElement("div");
    el.className = "sky-wrapper";
    ta.parentNode.insertBefore(el, ta);
    // Call update to actually insert line numbers to the wrapper
    SKY.update_line_numbers(ta, el);
